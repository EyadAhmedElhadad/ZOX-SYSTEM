...
export default function RateCustomerModal({
  ...
  const handleSubmit = async () => {
    ...
    // Backend integration point: POST /api/customers/:id/evaluation with { rating, reasons, notes, room, game }
    await new Promise((r) => setTimeout(r, 600));
    toast.success(`Customer rating submitted for ${customer}`);
        ...
        <div className="px-5 pb-2">
          <div className="flex items-center gap-2 bg-muted/40 rounded-xl p-3 mb-4">
            <div className="w-8 h-8 bg-primary/20 rounded-full flex items-center justify-center flex-shrink-0">
              <span className="text-xs font-bold text-primary">{customer.charAt(0)}</span>
            </div>
            <div>
